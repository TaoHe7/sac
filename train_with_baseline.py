...
from rl_zoo3.exp_manager import ExperimentManager, ExperimentManagerCurriculum
from rl_zoo3.utils import ALGOS, StoreDict
import intersection_environment


def train() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--algo", help="RL Algorithm", default="sac", type=str, required=False, choices=list(ALGOS.keys()))
    parser.add_argument("--env", type=str, default="intersection-v0", help="environment ID")
    parser.add_argument("-tb", "--tensorboard-log", help="Tensorboard log dir", default="", type=str)
    parser.add_argument("--curriculum_training", type=bool, default=True, help="Whether to use curriculum training")
    ...
    parser.add_argument("--num-threads", help="Number of threads for PyTorch (-1 to use default)", default=24, type=int)
    ...
    parser.add_argument("--save-freq", help="Save the model every n steps (if negative, no checkpoint)", default=1000000, type=int)
    parser.add_argument(
        "--save-replay-buffer", help="Save the replay buffer too (when applicable)", action="store_true", default=False
    )
    parser.add_argument("-f", "--log-folder", help="Log folder", type=str, default="/home/blp/Desktop/RL_Guidance/trained_agent")
    ...
    parser.add_argument("--n-jobs", help="Number of parallel jobs when optimizing hyperparameters", type=int, default=30)
    parser.add_argument(
        "--sampler",
    ...
    if args.curriculum_training:
        ...
        exp_manager = ExperimentManagerCurriculum(
            args,
            args.algo,
            'intersection-v0',
            args.log_folder,
